...
struct node_t{
  ...
  void rot(bool isL){
    node_t *q=pp,*r=q->pp;
    q->push(),push();
    if(((isL?q->rp:q->lp)=(isL?lp:rp)))(isL ? lp : rp)->pp=q;
    (isL ? lp : rp)=q;
    q->pp=this, pp=r;
    if(r && r->lp==q)r->lp=this;
    if(r && r->rp==q)r->rp=this;
    q->update();
  }
  ...
  void splay(){
    while(!is_root()){
      node_t *q=pp;
      bool a = q->is_root() ?: q->pp->lp==q;
      if(q->is_root())rot(q->lp!=this);
      else if((a ? q->lp : q->rp)==this)q->rot(!a),rot(!a);
      else rot(a),rot(!a);
    }
    push();
    update();
